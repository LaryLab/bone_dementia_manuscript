...
tbl4 <- tbl4 %>%
  ...
  dplyr::mutate(Row = row_number(),
                ...
                Model = c('4', '4: E4', '4: no E4', '5', '5: age < 71', '5: age >= 71', '6'))
...
# init BMD
model <- coxph(Surv(SurvTime10, DemStatus10) ~ BMDInit + AgeInit + BMIInit + Sex + APOEStatus,data=data)
summary(model)
model <- coxph(Surv(SurvTime10, DemStatus10) ~ BMDInit + AgeInit + BMIInit + Sex + APOEStatus,data=data[data$APOEStatus==1,])
summary(model)
model <- coxph(Surv(SurvTime10, DemStatus10) ~ BMDInit + AgeInit + BMIInit + Sex + APOEStatus,data=data[data$APOEStatus==0,])
summary(model)
