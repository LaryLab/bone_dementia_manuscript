...
orig_data[['BMD']] <- orig_data[['BMD']] %>%
  left_join(., common_data[["Dates"]] %>% 
              dplyr::filter(IDTYPE == 0) %>%
              dplyr::select(RANID, IDTYPE, InitExamDate, BaseExamDate), 
            by = common_cols)
...
off_data[['BMD8']] <- off_data[['t_bmdhs_2008_1_0748s_19.sas7bdat']] %>%
  ...
  mutate(RANID = as.numeric(RANID),
         IDTYPE = as.numeric(IDTYPE)) %>%
  dplyr::select(RANID, IDTYPE, BaseExamDate, BMDBase)
...
df <- common_data[['BMD']] %>%
  inner_join(common_data[['Dates']] %>% 
               dplyr::select(-InitExamDate, -BaseExamDate), 
             by = common_cols) %>%
  ...
  mutate(
    ...
    # BoneLossQ4 = ifelse(as.numeric(BoneLossLowestQuartileLabels) == 4, 1, 0),
