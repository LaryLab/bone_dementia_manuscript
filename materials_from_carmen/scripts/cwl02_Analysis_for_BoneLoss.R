...
tbl2 <- tbl2 %>%
  ...
  dplyr::mutate(
    ...
    PrimaryBMD = case_when(
      ...
      Row == 3 ~ 'BMDBaseHighestQuartile',
      Row == 4 ~ 'BoneLoss',
      Row == 5 ~ 'BoneLossLowestQuartileLabels',
      Row == 6 ~ 'BoneLossLowestQuartile',
      TRUE ~ PrimaryBMD
    ),
...
saveRDS(do.call(rbind, tbl2_results), paste0(res_dir, '/BoneLoss_Table2.rds'))
...
tbl4 <- tbl4 %>%
  ...
  dplyr::mutate(Row = row_number(),
         PrimaryBMD = case_when(
           ...
           Row == nrow(.) ~ 'BoneLoss:BMDBaseHighestQuartile',
           TRUE ~ PrimaryBMD
         ),
         APOE = case_when(
           Row == 2 ~ 'APOEStatus:0',
...
# bone loss
model <- coxph(Surv(SurvTime10, DemStatus10) ~ BoneLoss + AgeBase + BMIBase + Sex + APOEStatus,data=data)
summary(model)
model <- coxph(Surv(SurvTime10, DemStatus10) ~ BoneLoss + AgeBase + BMIBase + Sex + APOEStatus,data=data[data$APOEStatus==1,])
summary(model)
model <- coxph(Surv(SurvTime10, DemStatus10) ~ BoneLoss + AgeBase + BMIBase + Sex + APOEStatus,data=data[data$APOEStatus==0,])
summary(model)

# BMD base
model <- coxph(Surv(SurvTime10, DemStatus10) ~ BMDBase + AgeBase + BMIBase + Sex + APOEStatus,data=data)
summary(model)
model <- coxph(Surv(SurvTime10, DemStatus10) ~ BMDBase + AgeBase + BMIBase + Sex + APOEStatus,data=data[data$APOEStatus==1,])
summary(model)
model <- coxph(Surv(SurvTime10, DemStatus10) ~ BMDBase + AgeBase + BMIBase + Sex + APOEStatus,data=data[data$APOEStatus==0,])
summary(model)

# init BMD
model <- coxph(Surv(SurvTime10, DemStatus10) ~ BMDInit + AgeBase + BMIBase + Sex + APOEStatus,data=data)
summary(model)
model <- coxph(Surv(SurvTime10, DemStatus10) ~ BMDInit + AgeBase + BMIBase + Sex + APOEStatus,data=data[data$APOEStatus==1,])
summary(model)
model <- coxph(Surv(SurvTime10, DemStatus10) ~ BMDInit + AgeBase + BMIBase + Sex + APOEStatus,data=data[data$APOEStatus==0,])
summary(model)

# convert IQR to sd


model <- glm(DemStatus10~BoneLoss,data=data)
summary(model)

model <- coxph(Surv(data$SurvTime10, DemStatus10) ~ BMDBase + AgeBase + BMIBase + Sex + APOEStatus,data=data)
summary(model)

boxplot(BoneLoss~DemStatus10,data=data)
t.test(BoneLoss~DemStatus10,data=data)
summary(lm(SurvTime10~BoneLoss,data=data))
plot(SurvTime10~BoneLoss,data=data)
plot(SurvTime10~BoneLoss,data=data[data$DemStatus10==1,])
summary(lm(SurvTime10~BoneLoss,data=data[data$DemStatus10==1,]))
plot(SurvTime10~BoneLoss,data=data[data$DemStatus10!=1,])
summary(lm(SurvTime10~BoneLoss,data=data[data$DemStatus10!=1,]))
